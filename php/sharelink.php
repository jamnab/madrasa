<?php
	...
	if (isset($_REQUEST['text']) && ($_REQUEST['text'] != "")) {
		...
		$q = "SELECT * FROM posts WHERE $field1 LIKE '%" . implode("%' AND $field1 LIKE '%", $text) . "%' OR $field2 LIKE '%" . implode("%' AND $field2 LIKE '%", $text) . "%' OR $field3 LIKE '%" . implode("%' AND $field3 LIKE '%", $text) . "%' OR $field4 LIKE '%" . implode("%' AND $field4 LIKE '%", $text) . "%' ORDER BY RAND() LIMIT 150";
